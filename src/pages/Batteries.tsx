...
export default function Batteries() {
    ...
    const [newBattery, setNewBattery] = useState<Omit<Battery, 'id'>>({
        name: '',
        parttype: 'Battery',
        brand: '',
        quantity: 0,
    ...
    useEffect(() => {
        const fetchBatteries = async () => {
            try {
                const response = await fetch('http://localhost:5000/batteries/getBattery');
                if (!response.ok) {
    ...
    return (
        <div className="p-4">
            <div className="flex justify-between items-center mb-6">
                ...
                >
                    {showAddForm ? 'Cancel' : 'Add Battery'}
                </button>
            </div>
